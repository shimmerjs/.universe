...
## logiretch module (hidpp) -- DECIDED: direct-Bluetooth first, MX Master 4, battery leads
...
**logiretch-0 spike (<1 day, replaces the ~2-day receiver spike):**
...
**phase-1 logiretch capability: battery surfacing, read-only.** Feature query: root 0x0000 -> getFeature(0x1004 UNIFIED_BATTERY) to resolve its index (fallback 0x1000 BATTERY_STATUS) -> getStatus -> state-of-charge % + charging state. Poll on a bounded 60-300s interval, never per-tick (constant-cost); single direct node = one device, no receiver fan-out. Published as a retained state-shaped line on logiretch.sock. It sends one request and reads the reply -- module-internal, does NOT need the client-facing command lane.
...
**app-only, do NOT design for** [corrected 2026-07-14, per the verified capability matrix]: Logi Flow (LAN protocol; the device half 0x1814 fits phase 4 as manual host-switch), AI Prompt Builder / cloud sync, firmware DFU (fallback: temporary Options+ install per update). RECLASSIFIED host-reimplementable (consumers over logiretch.sock, zero touchd change): per-app profile switching (NSWorkspace frontmost watcher, khudson-side), Smart Actions / macros (divert CID -> Go action), Actions Ring (our own overlay on diverted CID 0x01A0 + 0x19B0 haptic tick; the Options+ ring UI itself is app-rendered, no wire protocol).
...
5. **logiretch module.** DECIDED direct-BT MX Master 4; logiretch-0 spike PASSED on-device 2026-07-15 (GO). Building now (Options+ removal cut): battery read + config-driven capability apply (DPI 0x2201 / SmartShift 0x2111 / hi-res wheel 0x2121 / thumbwheel 0x2150 / haptic 0x19B0 / static button remaps 0x1B04), applied module-internally on connect and on 0x1D4B reconfNeeded (no onboard profiles -> re-assert is the only persistence), plus a takeover reset of Options+ divert residue. events on logiretch.sock. The GESTURE/DIVERT ENGINE (phase 6a/6b: rawXY divert -> host gesture state machine -> CGEvent synth for the gesture button + Actions Ring) is LEDGERED/DEFERRED per the user 2026-07-15 -- see logi-replacement-design.md phase 6. Live client command lane (broadcaster v2 + Commander) and Bolt/receiver path deferred entirely.
...
- **C (feature bus)**: stressed separately after the first review truncated it. same standalone-consumer fatal (its own claims concede the thin client is "enabled, not shipped"); logitech lenses found the cross-node reply hazard unhandled and replies evictable; migration forces the proven gesture hot path through a brand-new single-socket read side. its four good mechanisms are grafted above; the feature taxonomy itself buys nothing once logiretch.sock carries per-device ids -- receiver multiplexing is 6 id values, and cross-topic aggregation is a topology fact not worth the taxonomy seam churn.
