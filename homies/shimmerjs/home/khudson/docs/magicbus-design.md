...
5. **logiretch module.** DECIDED direct-BT MX Master 4, battery-first (read-only, phase-1 logiretch capability above) behind the <1-day logiretch-0 spike; button/DPI/gesture config deferred to the command path. events on logiretch.sock. Bolt/receiver path deferred entirely.
