...
- **Fork execution; seed it with the spec, not the thread.** With `CLAUDE_CODE_SUBAGENT_MODEL` pinning every subagent to one model, a fan-out runs that same model anyway -- run the execution stage as a subagent whose only context is the written spec, so design exploration and the stress-test transcript don't pollute it.
