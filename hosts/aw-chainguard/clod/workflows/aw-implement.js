...
const exec = await named('execute', specText +
  ...
  { phase: 'Execute', isolation: iso, schema: EXEC })
...
const votes = (await parallel(Array.from({ length: flags.verify }, (_, i) => () =>
  named('verify:' + (i + 1), 'Skeptic ' + (i + 1) + ': does the implementation actually satisfy the spec and its acceptance criteria? Default meetsSpec=false unless you confirm by reading the real diff/code.' +
    '\nSPEC:\n' + spec.spec + '\nACCEPTANCE: ' + (spec.acceptance || []).join(' ; ') +
    '\nEXECUTION SUMMARY: ' + exec.summary + '\nFiles: ' + (exec.filesChanged || []).join(', ') + where,
    { phase: 'Verify', agentType: SKEPTIC, schema: VERDICT }))) ).filter(Boolean)
...
// codex finder leg (review phase ONLY -- codex stays out of the write path):
// cross-model recall on the produced diff; the reviewer's reconciled verdict
// supplies the precision, treating codex findings as candidates to confirm or
// reject, never pre-confirmed. The leg is a Claude relay (REVIEWER has Bash;
// codex exec is allowlisted) that must ground every citation before relaying.
// A dead leg (auth, exec error) is logged and tallied as codexDown -- kept
// distinguishable from "codex found nothing"; the review still runs.
let review = null, codexDown = false, codexDropped = 0
if (flags.review !== 'off') {
  phase('Review')
  ...
  let codexNote = ''
  if (flags.codex === 'on') {
    const cx = await named('review:codex',
      'Cross-model finder leg: relay codex, do NOT add findings of your own.\n' +
      '1. Run codex from the tree that holds the change' + (exec.worktree ? ' (worktree: ' + exec.worktree + ')' : '') + ':\n' +
      '   codex exec -s read-only -o <mktemp-out> "Review the uncommitted diff (git diff; files: ' + (exec.filesChanged || []).join(', ') + ') for correctness / security / perf defects, each with file:line and severity critical|high|med|low. Be adversarial and concrete; no style nits, no praise."\n' +
    ...
    if (!cx || cx.available === false) {
      codexDown = true
      log('codex finder leg DOWN: ' + (cx ? (cx.error || 'no error text') : 'agent died'))
    } else {
      codexDropped = cx.dropped || 0
      const cf = cx.findings || []
      if (cf.length) codexNote = '\nCross-model candidates from codex (grounded file:line, NOT pre-confirmed -- confirm or reject each in the verdict):\n' +
        cf.map(f => f.file + (f.line ? ':' + f.line : '') + ' [' + f.severity + '] ' + f.desc).join('\n')
    }
  }
  review = await named('review', 'Review the diff this change introduced for correctness / security / perf bugs (file:line, concrete, no style nits). Apply ~/.claude/workflows/partials/SYNTHESIS.md: confirmed-only, name the trade-off, one reconciled verdict.' +
    '\nEXECUTION SUMMARY: ' + exec.summary + '\nFiles: ' + (exec.filesChanged || []).join(', ') + where + codexNote,
    { phase: 'Review', agentType: REVIEWER })
}
